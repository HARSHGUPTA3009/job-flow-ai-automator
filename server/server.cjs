...
app.use(session({
  ...
  cookie: {
    httpOnly: true,
    secure: true,
    sameSite: 'none', // ✅ lowercase 'none' required for cross-site cookies
...
app.post('/ai/cold-emails', async (req, res) => {
  try {
    ...
    if (!parsed.emails || !Array.isArray(parsed.emails)) {
      return res.status(500).json({ error: 'Invalid cold email structure', raw: parsed });
    }

    return res.json({ emails: parsed.emails });

  } catch (err) {
    console.error('Groq Cold Email Error:', err.response?.data || err.message);
    return res.status(500).json({ error: 'Cold email AI processing failed', details: err.response?.data || err.message });
  }
});
...
const OffCampusApplicationSchema = new mongoose.Schema({
  ...
  status:            { type: String, enum: ['applied','screening','interview','offer','rejected','accepted'], default: 'applied' },
  notes:             String,
  followUpDates:     [String],
  source:            { type: String, enum: ['linkedin','indeed','naukri','direct','other'], default: 'linkedin' },
...
const OnCampusApplicationSchema = new mongoose.Schema({
  ...
  role:           String,
  appliedDate:    String,
  status:         { type: String, enum: ['applied','shortlisted','interview_1','interview_2','interview_3','rejected','offer'], default: 'applied' },
  ...
  try {
    ...
    uploadStream.on('finish', async () => {
      try {
        const profile = await Profile.findOneAndUpdate(
          { userId },
          { $push: { resumes: { fileId: uploadStream.id.toString(), name: file.originalname, uploadDate: new Date(), isActive: false } } },
          { new: true, upsert: true }
        );
        res.json({ message: 'Resume uploaded successfully', fileId: uploadStream.id.toString(), resumes: profile.resumes });
      } catch (error) {
        res.status(500).json({ error: 'Failed to update profile' });
      }
    });
    uploadStream.on('error', () => res.status(500).json({ error: 'Failed to upload file' }));
  } catch (error) {
...
app.delete('/api/placement/resume/:fileId', async (req, res) => {
  try {
    ...
    const db     = mongoose.connection.db;
    const bucket = new GridFSBucket(db, { bucketName: 'resumes' });
    try { await bucket.delete(new mongoose.Types.ObjectId(fileId)); } catch (_) {}
    ...
    res.json({ message: 'Resume deleted successfully', resumes: profile?.resumes || [] });
  } catch (error) {
...
app.get('/api/placement/resume/download/:fileId', async (req, res) => {
  try {
    ...
    const files = await db.collection('resumes.files').find({ _id: new mongoose.Types.ObjectId(fileId) }).toArray();
    ...
    downloadStream.on('error', () => { if (!res.headersSent) res.status(404).json({ error: 'File not found' }); });
    downloadStream.pipe(res);
  } catch (error) {
    if (!res.headersSent) res.status(500).json({ error: 'Failed to download resume' });
  }
});

// ============================================================================
// PLACEMENT TRACKER — OFF-CAMPUS
// ============================================================================

app.get('/api/placement/off-campus/:userId', async (req, res) => {
  try {
    const applications = await OffCampusApplication.find({ userId: req.params.userId }).sort({ createdAt: -1 });
...
app.post('/api/placement/off-campus', async (req, res) => {
  try {
    if (!req.body.userId || !req.body.company || !req.body.jobTitle)
      return res.status(400).json({ error: 'userId, company, and jobTitle are required' });
    const application = await new OffCampusApplication(req.body).save();
...
// ============================================================================
// PLACEMENT TRACKER — ON-CAMPUS
// ============================================================================

app.get('/api/placement/on-campus/:userId', async (req, res) => {
  try {
    const applications = await OnCampusApplication.find({ userId: req.params.userId }).sort({ createdAt: -1 });
    res.json(applications);
  } catch { res.status(500).json({ error: 'Failed to fetch applications' }); }
});
...
app.post('/api/placement/on-campus', async (req, res) => {
  try {
    ...
    const application = await new OnCampusApplication(req.body).save();
    res.status(201).json(application);
  } catch { res.status(500).json({ error: 'Failed to create application' }); }
});
...
app.put('/api/placement/on-campus/:id', async (req, res) => {
  try {
    const application = await OnCampusApplication.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
    if (!application) return res.status(404).json({ error: 'Application not found' });
    res.json(application);
  } catch { res.status(500).json({ error: 'Failed to update application' }); }
});
...
// ============================================================================
// PLACEMENT TRACKER — COMPANY DRIVES
// ============================================================================

app.get('/api/placement/company-drives/:userId', async (req, res) => {
  try {
    const drives = await CompanyDrive.find({ userId: req.params.userId }).sort({ createdAt: -1 });
    res.json(drives);
  } catch { res.status(500).json({ error: 'Failed to fetch company drives' }); }
});
...
app.post('/api/placement/company-drives', async (req, res) => {
  try {
    if (!req.body.userId || !req.body.companyName)
      return res.status(400).json({ error: 'userId and companyName are required' });
    const drive = await new CompanyDrive(req.body).save();
    res.status(201).json(drive);
  } catch { res.status(500).json({ error: 'Failed to create company drive' }); }
});

app.put('/api/placement/company-drives/:id', async (req, res) => {
  try {
    const drive = await CompanyDrive.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
    if (!drive) return res.status(404).json({ error: 'Company drive not found' });
    res.json(drive);
  } catch { res.status(500).json({ error: 'Failed to update company drive' }); }
});
...
app.delete('/api/placement/company-drives/:id', async (req, res) => {
  ...
  } catch { res.status(500).json({ error: 'Failed to delete company drive' }); }
});
...
app.get('/health', (req, res) => {
  res.json({ status: 'running', timestamp: new Date(), uptime: process.uptime(), mongodb: mongoose.connection.readyState === 1 ? 'connected' : 'disconnected' });
});
